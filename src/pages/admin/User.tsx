...
export default function UserManagement() {
  ...
  const { totalUsers, totalUsersThisMonth, totalUsersVerified } = useUserData();
  ...
  const fetchJobs = async () => {
    try {
      const response = await apiService.get<ApiResponse<Job[]>>(
        "/jobs/get-all"
      );
  ...
  const fetchContracts = async () => {
    try {
      const response = await apiService.get<ApiResponse<Contract[]>>(
        "/contracts/get-all"
      );
  ...
  const fetchEmployees = async (employeeId: string) => {
    ...
    try {
      const response = await apiService.get<{ data: Employee }>(
    ...
    if (selectedEmployee) {
      employeeForm.setFieldsValue({
        ...
        start_date: selectedEmployee.start_date
          ? dayjs(selectedEmployee.start_date)
          : null,
        end_date: selectedEmployee.end_date
          ? dayjs(selectedEmployee.end_date)
          : null,
      });
    }
  ...
  const handleChangeUserStatus = async (userId: string, isBlocked: boolean) => {
    try {
      ...
      message.success(
        `User ${isBlocked ? "banned" : "unbanned"} successfully!`
      );
  ...
  const handleChangeUserRole = async () => {
    ...
    try {
      ...
      const updatedUser = { ...editingUser, role_code: selectedRole };
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_: unknown, record: User) =>
        roleMap[record.role_code] || record.role_code,
    },
    {
      title: (
        <div className="flex items-center gap-2">
    ...
    },
    {
      ...
      render: (_: string, record: User) => (
        <div className="relative">
          <button
            className={`p-2 rounded-lg transition-all duration-500 w-24 border ${
              record.is_blocked
            ...
            onMouseEnter={(e) =>
              (e.currentTarget.innerText = record.is_blocked ? "Unban" : "Ban")
            }
            onMouseLeave={(e) =>
              (e.currentTarget.innerText = record.is_blocked
                ? "Banned"
                : "Active")
            }
            onClick={() => {
              Modal.confirm({
                ...
                content: `Do you want to ${
                  record.is_blocked ? "unban" : "ban"
                } the user ${record.user_name}?`,
                onOk: () =>
                  handleChangeUserStatus(record._id, !record.is_blocked),
                okText: "Confirm",
                cancelText: "Cancel",
              });
            }}
      ...
      <div className="relative min-h-screen">
        ...
        <div className="bg-[#FCFCFC] p-5">
          ...
          >
            ...
            {showUserCards ? <KeyboardArrowUp /> : <KeyboardArrowDown />}
          </div>
                  ...
                  >
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        employeeForm.setFieldsValue({ phone: formatted });
                      }}
                    />
                  </Form.Item>
                </Col>
                    ...
                    rules={[
                      {
                        ...
                        validator: (_, value) => {
                          if (value <= 0) {
                            return Promise.reject("Salary must be positive");
                          }
                          return Promise.resolve();
                        },
                      },
                ...
                <Col span={12}>
                  ...
                  >
                    <DatePicker
                      format="YYYY-MM-DD"
                      value={
                        employeeForm.getFieldValue("start_date")
                          ? dayjs(employeeForm.getFieldValue("start_date"))
                          : null
                      }
                      onChange={(date) =>
                        employeeForm.setFieldsValue({ start_date: date })
                      }
                      style={{ width: "100%" }}
                    />
                  </Form.Item>
                ...
                <Col span={12}>
                  ...
                  >
                    <DatePicker
                      format="YYYY-MM-DD"
                      value={
                        employeeForm.getFieldValue("end_date")
                          ? dayjs(employeeForm.getFieldValue("end_date"))
                          : null
                      }
                      onChange={(date) =>
                        employeeForm.setFieldsValue({ end_date: date })
                      }
                      style={{ width: "100%" }}
                    />
                  </Form.Item>
