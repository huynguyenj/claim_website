...
import { Space, Form, Input, Button, Modal, Divider, Card, Spin, message } from 'antd'
...
function Profile() {
    ...
    const fetchClaims = async () => {
        ...
        const pendingClaims = totalClaims.filter((claim) => (claim.claim_status == 'Pending Approval'))
        setPendingClaims(pendingClaims)

        const approvedClaims = totalClaims.filter((claim) => (claim.claim_status == 'Approved'))
        setApprovedClaims(approvedClaims)

        const rejectedClaims = totalClaims.filter((claim) => (claim.claim_status == 'Rejected'))
            ...
            <div className="w-3/4 grid grid-cols-1 lg:grid-cols-2 gap-10">
                ...
                shadow-[2px_2px_0px_black]">
                    ...
                    <Spin tip="Loading" size="large" spinning={fetchingClaims}>
                        <ul className='flex flex-col items-center'>
                            <Button
                            style={{padding: "1rem", margin: "1rem", borderRadius: "0.5rem", gridTemplateColumns: "repeat(6, minmax(0, 1fr))", boxShadow:'2px 2px 0px black'}}
                            onClick={() => {
                        ...
                        {claims.map((claim) => (
                            ...
                            onClick={()=>{
                                ...
                                <div className="grid grid-cols-3 rounded-md">
                                    <span className='col-span-2 font-bold text-2xl'>{claim.claim_name}</span>
                ...
                >
                    <Form>
                        ...
                        <Form.Item
                        label="Start Date">
                            <Input disabled value={formatDate(selectedClaim?.claim_start_date)} />
                        </Form.Item>
                        <Form.Item
                        label="End Date">
                            <Input disabled value={formatDate(selectedClaim?.claim_start_date)} />
                        ...
                        label="Total Work Time">
                            <Input disabled value={`${selectedClaim?.total_work_time} hours`} />
                        </Form.Item>
                        <Form.Item
                        label="Status">
